      ...
      {items.length === 0 ? (
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.productId} className="flex gap-4 rounded-md border border-border bg-card p-4">
                <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-md bg-surface">
                  <Image src={item.image} alt={item.name} fill className="object-cover" sizes="96px" />
                </div>
                <div className="flex flex-1 flex-col">
                  <Link href={`/product/${item.slug}`} className="text-heading-lg font-semibold hover:text-primary">
                    {item.name}
                  </Link>
                  ...
                  <div className="mt-auto flex items-center justify-between pt-2">
                    ...
                    <p className="text-heading-lg font-bold">{formatCurrency(item.price * item.quantity)}</p>
                  </div>
                </div>
                <button
                  type="button"
          ...
          <div className="h-fit rounded-lg border border-border bg-card p-6 shadow-sm">
            ...
            <Link href="/rfq" className="mt-3 block">
              <Button variant="outline" className="w-full">Request Bulk Quote Instead</Button>
            </Link>
          </div>
        </div>
      )}
